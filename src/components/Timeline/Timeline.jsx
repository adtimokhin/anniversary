...
const Timeline = ({ events }) => {
  ...
  const totalTimeSpan = events.reduce((acc, event, index) => {
    ...
    const timeSpan = (event.year - prevEvent.year) * 12 + (event.month - prevEvent.month);
    return acc + timeSpan;
  }, 0);
  ...
  return (
    <div className="timeline">
      {events.map((event, index) => {
        ...
        const timeSpan = prevEvent ? (event.year - prevEvent.year) * 12 + (event.month - prevEvent.month) : 0;
        const marginLeft = prevEvent ? `${(timeSpan / totalTimeSpan) * 100}%` : 0;
